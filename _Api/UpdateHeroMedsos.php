<?php
    ...
    if ($title === '') {
        respond(['status' => 'error', 'message' => 'Title wajib diisi'], 422);
    }

    if (!is_array($sub_title)) {
        respond(['status' => 'error', 'message' => 'sub_title harus berupa array'], 422);
    }

    // Validasi struktur sub_title
    foreach ($sub_title as $item) {
        if (!isset($item['url'], $item['icon'], $item['order'])) {
            respond(['status' => 'error', 'message' => 'Setiap item sub_title harus memiliki url, icon, dan order'], 422);
        }
    }
    ...
    $oldImage = $data['hero']['hero_media_sosial']['image'] ?? null;
    $newImageName = $oldImage;

    // Proses gambar jika diberikan
    ...
    // Update nilai hero_media_sosial
    $data['hero']['hero_media_sosial'] = [
        'image' => $newImageName,
        'title' => $title,
        'sub_title' => $sub_title
    ];
    ...
    respond([
        'status' => 'success',
        'message' => 'Hero Media Sosial berhasil diperbarui',
        'hero_media_sosial' => $data['hero']['hero_media_sosial']
    ]);
?>