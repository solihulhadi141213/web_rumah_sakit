<?php
    ...
    // Ambil data galeri item dari database
    $stmtGaleri = $Conn->prepare("SELECT * FROM galeri_item WHERE id_galeri='$id_galeri' ORDER BY id_galeri_item DESC LIMIT :limit OFFSET :offset");
    ...
    foreach ($galeriList as $row) {
        ...
        $proxy_cover="$base_url/image_proxy.php?segment=Galeri&image_name=$file_item";
        $data[] = [
            'id_galeri_item' => $row['id_galeri_item'],
            'id_galeri' => $row['id_galeri'],
            'title_item' => $row['title_item'],
            'description_item' => $row['description_item'],
            'file_item' => $proxy_cover,
    ...
    sendResponse([
        'status' => 'success',
        'message' => 'Galeri Item berhasil ditampilkan.',
        'page' => $page,
        'limit' => $limit,
        'data' => $data
    ], 200);
